...
def Astar(img, node, goal, neighbours, map, parentTracker, vis, start):
    ...
    for i in neighbours[node]:
        ...
        h_score = abs(goal[0] - i[0][0]) + abs(goal[1] - i[0][1])
    ...
    for i in neighbours[node]:
        ...
        if f_score<min and i[0] not in vis:
            min = f_score
            next_node = i[0]
    if allNeighboursInVisited(neighbours[node], vis):
        next_node = parentTracker[node]
        for i in neighbours[next_node]:
            if i[0]==node:
                g_score = i[1]
        neighbours[next_node].remove((node, g_score))
